<x-form.modal title="{{ __('translation.app-profil') }}" action="{{ $action ?? null }}" enctype="multipart/form-data">
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            ...
            @if ($data->app_icon && file_exists(base_path('build/images/' . $data->app_icon)))
                <img src="{{ asset('build/images/' . $data->app_icon) }}" width="50" alt="Icon Aplikasi" />
            @elseif ($data->app_icon && file_exists(base_path('images/' . $data->app_icon)))
            ...
            @if ($data->app_logo && file_exists(base_path('build/images/' . $data->app_logo)))
                <img src="{{ asset('build/images/' . $data->app_logo) }}" alt="Logo Aplikasi" />
            @elseif ($data->app_logo && file_exists(base_path('images/' . $data->app_logo)))
