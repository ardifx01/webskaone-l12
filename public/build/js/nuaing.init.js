...
/* function SearchData() {
  ...
  var isjenis = document.getElementById("idJenis").value;
  var isstatus = document.getElementById("idStatus").value;
  customerList.filter(function (data) {
    matchDataJenisPersonil = new DOMParser().parseFromString(data.values().jenispersonil, 'text/html');
    var jenispersonil = matchDataJenisPersonil.body.firstElementChild.innerHTML;
    var jenisFilter = false;

    matchDataStatus = new DOMParser().parseFromString(data.values().status, 'text/html');
    var status = matchDataStatus.body.firstElementChild.innerHTML;
    var statusFilter = false;

    if (jenispersonil == 'all' || isjenis == 'all' || status == 'all' || isstatus == 'all') {
        jenisFilter = true;
        statusFilter = true;

    } else {
        jenisFilter = jenispersonil == isjenis;
        statusFilter = status == isstatus;
    }

    if (jenisFilter && statusFilter) {
        return jenisFilter && statusFilter
    } else if (jenisFilter && statusFilter == "") {
        return jenisFilter
    } else if (statusFilter && jenisFilter == "") {
        return statusFilter
    }
  ...
  function SearchData() {
    // Ambil nilai yang dipilih untuk jenis dan status
    var isjenis = document.getElementById("idJenis").value;
    var isstatus = document.getElementById("idStatus").value;

    // Log untuk memeriksa nilai yang dipilih dari dropdown
    /*     console.log('Jenis yang dipilih:', isjenis);
    console.log('Status yang dipilih:', isstatus); */

    // Filter daftar customer
    customerList.filter(function (data) {
        // Parsing nilai status dan jenis dari data
        var matchStatusData = new DOMParser().parseFromString(data.values().aktif, 'text/html');
        var status = matchStatusData.body.firstElementChild ? matchStatusData.body.firstElementChild.innerHTML.trim().toLowerCase() : '';

        var matchJenisData = new DOMParser().parseFromString(data.values().jenispersonil, 'text/html');
        var jenis = matchJenisData.body.firstElementChild ? matchJenisData.body.firstElementChild.innerHTML.trim().toLowerCase() : '';

        // Log untuk memeriksa nilai dari dataset
        /* console.log('Jenis dari data:', jenis);
        console.log('Status dari data:', status); */

        var statusFilter = false;
        var jenisFilter = false;

        // Filter berdasarkan status
        if (isstatus === 'all' || isstatus === "") {
            statusFilter = true;
        } else {
            statusFilter = status === isstatus.toLowerCase();
        }

        // Filter berdasarkan jenis
        if (isjenis === 'all' || isjenis === "") {
            jenisFilter = true;
        } else {
            jenisFilter = jenis === isjenis.toLowerCase();
        }

        // Log untuk memeriksa apakah filter berhasil
        /* console.log('Jenis cocok:', jenisFilter);
        console.log('Status cocok:', statusFilter); */

        // Kembalikan hasil berdasarkan pilihan:
        if (isstatus && isjenis) {
            return statusFilter && jenisFilter;
        } else if (isstatus && !isjenis) {
            return statusFilter;
        } else if (isjenis && !isstatus) {
            return jenisFilter;
        } else {
            return true;
        }
    ...
    // Perbarui daftar customer setelah filter
    customerList.update();
}
